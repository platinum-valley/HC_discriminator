...
class Attention(nn.Module):
    ...
    def __init__(self, hidden_dim):
        ...
        """
        self.linear1 = nn.Linear(self.hidden_dim*2, 1)
        self.softmax = nn.Softmax(dim=2)


    def forward(self, hidden, encoder_output, device):
        concatenated_attn = torch.cat((hidden, encoder_output), dim=2).type(torch.FloatTensor).to(device)
        ...
        attn_weights = self.linear1(concatenated_attn).to(device)
        ...
        encoder_output = torch.transpose(encoder_output, 0, 1).to(device)
        ...
        attn_hidden = torch.bmm(attn_weights.type(torch.FloatTensor).to(device), encoder_output.type(torch.FloatTensor).to(device))
        #attn_hidden = Variable(torch.squeeze(attn_hidden)).to(device)
        attn_hidden = torch.squeeze(attn_hidden).to(device)
        return attn_hidden
